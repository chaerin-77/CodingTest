...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        temp1 = dp_M[1];
        temp2 = dp_M[2];
        dp_M[0] = max(temp0, temp1) + input1;
        dp_M[1] = max({temp0, temp1, temp2}) + input2;
        dp_M[2] = max(temp1, temp2) + input3;

        temp0 = dp_m[0];
        temp1 = dp_m[1];
        temp2 = dp_m[2];
        dp_m[0] = min(temp0, temp1) + input1;
        dp_m[1] = min({temp0, temp1, temp2}) + input2;
        dp_m[2] = min(temp1, temp2) + input3;
    }

    cout << max({dp_M[0], dp_M[1], dp_M[2]}) << " " << min({dp_m[0], dp_m[1], dp_m[2]}) << endl;
    return 0;
}